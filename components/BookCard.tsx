...
const BookCard = ({ title, author, coverURL, slug }: BookCardProps) => {
  return (
    <Link href={`/books/${slug}`} className="block group">
      <article className="flex flex-col w-60 bg-white rounded-2xl p-4 shadow-sm border border-gray-100 hover:shadow-lg hover:border-gray-200 transition-all duration-300 h-full cursor-pointer">
          ...
          <div className="relative w-full flex justify-center mb-4">
            <div className="overflow-hidden rounded-md shadow-sm group-hover:shadow-md transition-all duration-300">
              <Image
                ...
                width={133}
                height={200}
                className="object-cover w-[133px] h-[200px] group-hover:scale-105 transition-transform duration-500 ease-out"
              />
            </div>
          </div>
